...
const { consumeStock, reverseStockForOrder } = require("../src/lib/stock");
...
async function main() {
  ...
  const tableIds = {};
  for (let i = 1; i <= 8; i++) {
    const record = await prisma.restaurantTable.upsert({
      where: { number: i },
      update: {},
  ...
  const priceByName = Object.fromEntries(items.map((i) => [i.name, i.price]));

  await seedDemoOrders({ priceByName, menuItemIds, userIds, tableIds });
...
const demoOrderSpecs = [
  {
    type: "DINE_IN",
    table: 1,
    createdBy: "waiter@restaurant.com",
    status: "CONFIRMED",
    items: [
      { name: "Bruschetta al Pomodoro", quantity: 1, status: "PENDING" },
      { name: "Limonada com Gás", quantity: 2, status: "PENDING" },
    ],
  },
  {
    type: "DINE_IN",
    ...
    status: "PREPARING",
    items: [
      { name: "Pizza Margherita", quantity: 1, status: "PREPARING" },
      { name: "Salada Caesar", quantity: 1, status: "READY" },
    ],
  },
  {
    type: "DINE_IN",
    table: 5,
    createdBy: "admin@restaurant.com",
    status: "READY",
    items: [
      { name: "Bife Ancho", quantity: 1, status: "READY" },
      { name: "Vinho Tinto da Casa (Taça)", quantity: 1, status: "READY" },
    ],
  },
  {
    type: "DINE_IN",
    ...
    createdBy: "waiter@restaurant.com",
    status: "SERVED",
    items: [{ name: "Fettuccine Alfredo", quantity: 2, status: "READY" }],
  },
  {
    type: "TAKEAWAY",
    createdBy: "admin@restaurant.com",
    ...
    status: "PREPARING",
    items: [{ name: "Salmão Grelhado", quantity: 1, status: "PREPARING" }],
  },
  {
    type: "TAKEAWAY",
    createdBy: "waiter@restaurant.com",
    ...
    status: "COMPLETED",
    items: [{ name: "Tiramisù", quantity: 2, status: "READY" }],
  },
  {
    type: "DELIVERY",
    createdBy: "admin@restaurant.com",
    customerName: "Rafael Nogueira",
    customerPhone: "(11) 96666-4321",
    deliveryAddress: "Rua das Palmeiras, 245 - Apto 12",
    status: "READY",
    items: [
      { name: "Pizza Margherita", quantity: 2, status: "READY" },
      { name: "Cola Artesanal", quantity: 2, status: "READY" },
    ],
  },
  {
    type: "DELIVERY",
    ...
    customerPhone: "(11) 95555-8765",
    deliveryAddress: "Av. Brasil, 900 - Bloco B",
    status: "OUT_FOR_DELIVERY",
    driver: "delivery@restaurant.com",
    items: [
      { name: "Lula Empanada", quantity: 1, status: "READY" },
      { name: "Cola Artesanal", quantity: 1, status: "READY" },
    ],
  },
  {
    type: "DELIVERY",
    ...
    customerPhone: "(11) 94444-3456",
    deliveryAddress: "Rua Sete de Setembro, 55",
    status: "DELIVERED",
    driver: "delivery@restaurant.com",
    items: [{ name: "Bolo Vulcão de Chocolate", quantity: 2, status: "READY" }],
  },
  {
    type: "DINE_IN",
    table: 8,
    createdBy: "waiter@restaurant.com",
    status: "CANCELLED",
    items: [{ name: "Lula Empanada", quantity: 1, status: "PENDING" }],
  },
];
...
async function createDemoOrder(
  tx,
  spec,
  { priceByName, menuItemIds, userIds, tableIds },
) {
  const tableId = spec.table ? tableIds[spec.table] : null;
  const total = spec.items.reduce(
    (sum, i) => sum + priceByName[i.name] * i.quantity,
    0,
  );
  ...
  const order = await tx.order.create({
    data: {
      ...
      createdById: userIds[spec.createdBy],
      driverId: spec.driver ? userIds[spec.driver] : null,
      total,
      items: {
        create: spec.items.map((i) => ({
          menuItemId: menuItemIds[i.name],
          quantity: i.quantity,
          unitPrice: priceByName[i.name],
          status: i.status,
        })),
      },
    },
  ...
  const orderItems = spec.items.map((i) => ({
    menuItemId: menuItemIds[i.name],
    quantity: i.quantity,
  }));

  await consumeStock(tx, orderItems, {
    ...INGREDIENT_RECIPE,
    orderId: order.id,
  });
  ...
  const packagingConsumed = ["OUT_FOR_DELIVERY", "DELIVERED"].includes(
    spec.status,
  );
  if (spec.type === "DELIVERY" && packagingConsumed) {
    await consumeStock(tx, orderItems, {
      ...PACKAGING_RECIPE,
      orderId: order.id,
    });
  ...
  if (spec.status === "CANCELLED") {
    await reverseStockForOrder(tx, order.id);
  }

  if (tableId && ["CANCELLED", "COMPLETED"].includes(spec.status)) {
    await tx.restaurantTable.update({
      where: { id: tableId },
      data: { status: "FREE" },
    });
...
async function seedDemoOrders({ priceByName, menuItemIds, userIds, tableIds }) {
  const existingOrders = await prisma.order.count();
  if (existingOrders > 0) {
    console.log("Pedidos de demonstração já existem — pulando.");
    return;
  }

  console.log("Semeando pedidos de demonstração...");
  for (const spec of demoOrderSpecs) {
    await prisma.$transaction((tx) =>
      createDemoOrder(tx, spec, {
        priceByName,
        menuItemIds,
        userIds,
        tableIds,
