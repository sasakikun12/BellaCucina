...
vi.mock("../../src/api/orders", () => ({ fetchOrders: vi.fn(), setOrderStatus: vi.fn() }));
...
// NewOrderForm é testado à parte; aqui um stub que só sinaliza abertura/fecho.
...
describe("Orders", () => {
  ...
  it("aba Ativos: READY no salão -> 'Marcar como servido'; cancela", async () => {
    ...
    await user.click(await screen.findByRole("button", { name: /Marcar como servido/i }));
  ...
  it("READY para viagem -> 'Marcar como concluído'", async () => {
    fetchOrders.mockResolvedValue([ord({ status: "READY", type: "TAKEAWAY", table: null })]);
    const user = userEvent.setup();
    renderWithProviders(<Orders />);
    await user.click(await screen.findByRole("button", { name: /Marcar como concluído/i }));
  ...
  it("READY de entrega não mostra botão de servir/concluir (só cancelar)", async () => {
    fetchOrders.mockResolvedValue([ord({ status: "READY", type: "DELIVERY", table: null })]);
    renderWithProviders(<Orders />);
    await screen.findByText("Cancelar");
    expect(screen.queryByRole("button", { name: /Marcar como/i })).not.toBeInTheDocument();
  });
