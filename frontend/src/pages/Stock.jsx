...
function AdjustStockModal({ api, queryKey, item, onClose }) {
  ...
  // Reposição sempre soma ao estoque, Perda sempre subtrai — o sinal vem do
  // motivo escolhido, não de o usuário lembrar de digitar um número negativo.
  const adjustMutation = useMutation({
    mutationFn: () => api.adjust(item.id, reason === "RESTOCK" ? Number(delta) : -Number(delta), reason),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [queryKey] });
      onClose();
    },
...
function RecipesTab() {
  ...
  // Recarrega as linhas editáveis locais quando a receita de outro item termina de carregar.
