...
// Ingredientes e embalagens têm o mesmo formato de endpoints, então uma única
// fábrica monta os dois módulos de API.
function buildCatalogApi(basePath) {
  return {
    ...
    adjust: async (id, delta, reason) => {
      const { data } = await api.patch(`${basePath}/${id}/adjust`, { delta, reason });
      return data;
    },
  };
}
