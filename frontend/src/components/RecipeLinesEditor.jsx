...
export function RecipeLinesEditor({
  ...
  function updateQty(id, value) {
    setLines((prev) =>
      prev.map((l) =>
        l[idField] === id ? { ...l, quantityPerUnit: value } : l,
      ),
    );
  }
  ...
  return (
    <div className="rounded-xl border border-stone-200 bg-white p-4 shadow-sm">
      ...
      {lines.length === 0 && (
        <p className="mb-3 text-sm text-stone-500">{emptyLabel}</p>
      )}
      ...
      <ul className="mb-3 flex flex-col gap-2">
        {lines.map((line) => (
          <li key={line[idField]} className="flex items-center gap-2 text-sm">
            <span
              className="min-w-0 flex-1 truncate text-stone-700"
              title={line.name}
            >
              {line.name}
            </span>
            ...
            />
            <span className="w-8 shrink-0 text-xs text-stone-400">
              {line.unit}
            </span>
            <button
              type="button"
